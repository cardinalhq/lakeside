...
class ASTUtilsBaseExprTest {
  ...
  def testQueryApiPayloadWithExtract(): Unit = {
    ...
    val expectedExemplarSql = """SELECT "_cardinalhq.timestamp", "_cardinalhq.value", "_cardinalhq.name", "_cardinalhq.message", * FROM (SELECT nlp_struct['raw'] as raw, nlp_struct['compressed'] as compressed, * FROM (SELECT  regexp_extract(replace("_cardinalhq.message", '''', ''), '([A-Za-z]+) \| ([A-Za-z]+) \| ([A-Za-z]+) \| \(([^)]*)\) \| ([A-Za-z]+) ([A-Za-z]+) ([A-Za-z]+), ([A-Za-z]+) \(([^)]*)\)([A-Za-z0-9-_.:]+) ([0-9]+)/([0-9]+) ([A-Za-z0-9-_.:]+)', ['var_1', 'var_2', 'var_3', 'var_4', 'var_5', 'var_6', 'var_7', 'var_8', 'var_9', 'var_10', 'raw', 'compressed', 'var_13']) as nlp_struct, * FROM (SELECT * FROM {tableName} WHERE "_cardinalhq.timestamp" >= 1694635527646 AND "_cardinalhq.timestamp" < 1694635527646) WHERE  regexp_matches(replace("_cardinalhq.message", '''', ''), '([A-Za-z]+) \| ([A-Za-z]+) \| ([A-Za-z]+) \| \(([^)]*)\) \| ([A-Za-z]+) ([A-Za-z]+) ([A-Za-z]+), ([A-Za-z]+) \(([^)]*)\)([A-Za-z0-9-_.:]+) ([0-9]+)/([0-9]+) ([A-Za-z0-9-_.:]+)'))) WHERE ((("resource.container.name" = 'agent' and regexp_matches("_cardinalhq.message", '.*compressed.*','i')) and raw IS NOT NULL) and compressed IS NOT NULL) ORDER BY "_cardinalhq.timestamp" DESC LIMIT 1000"""
